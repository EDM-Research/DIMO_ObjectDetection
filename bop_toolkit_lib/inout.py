...
def load_ply(path):
    ...
    vertex_elem = plydata['vertex']
    faces_elem = plydata['face']

    pts = []
    faces = []

    for i in range(vertex_elem.count):
        pts.append([vertex_elem.data['x'][i], vertex_elem.data['y'][i], vertex_elem.data['z'][i]])

    for i in range(faces_elem.count):
        faces.append([faces_elem.data['vertex_indices'][i]])
